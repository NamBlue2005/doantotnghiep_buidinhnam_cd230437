...
import { availableOrdersState, orderRefreshKeyState } from "@/state";
...
export default function DriverHomePage() {
  const orderList = useAtomValue(
    useMemo(() => loadable(availableOrdersState), [])
  );
  ...
  const handleTouchEnd = () => {
    ...
    if (pullDistance > 60) {
      setRefreshing(true);
      setOrderRefreshKey(prev => prev + 1); // Kích hoạt gọi API tải lại đơn hàng
      setTimeout(() => {
        setRefreshing(false);
      }, 1000); // Ẩn loading sau 1s để UI mượt mà
    }
    setStartY(0);
    setPullDistance(0);
  };
  ...
  return (
    <div 
      ref={listRef}
      className="h-full flex flex-col bg-background overflow-y-auto p-4 relative"
    ...
    >
      ...
      <div className="space-y-2">
        <h2 className="text-lg font-bold text-primary mb-4">Đơn hàng quanh đây</h2>
