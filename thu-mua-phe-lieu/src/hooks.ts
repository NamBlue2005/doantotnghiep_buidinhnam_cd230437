...
export function useCheckout() {
  ...
  const refreshNewOrders = useSetAtom(orderRefreshKeyState);
  ...
  return async () => {
    try {
      ...
      await createOrder({
        ...
        mac: "dummy_mac_for_typescript", // Thêm mac giả để fix lỗi TypeScript
      });
      setCart([]);
      refreshNewOrders((prev) => prev + 1);
      navigate("/orders", {
        viewTransition: true,
      });
